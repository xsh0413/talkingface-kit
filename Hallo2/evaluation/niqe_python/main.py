...
def _get_patches_generic(img, patch_size, is_train, stride):
    ...
    # 使用Pillow来缩小图像
    img2 = Image.fromarray(img)  # 转换为PIL图像
    img2 = img2.resize((img2.width // 2, img2.height // 2), Image.BICUBIC)  # 缩小为原来的一半
    img2 = np.array(img2)  # 转回为numpy数组

    # 继续进行MSCN变换和特征提取等后续步骤
...
if __name__ == "__main__":
    ref = np.array(Image.open('./test_imgs/0325.png').convert('LA'))[:, :, 0]  # ref
    ...
    print(f'Time taken to compute NIQE score: {elapsed_time:.3f} seconds')
